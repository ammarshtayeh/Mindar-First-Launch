...
export default function QuizPage() {
  ...
  const [answers, setAnswers] = useState<
    Record<
      number,
      { selectedOption?: string; textAnswer?: string; isCorrect?: boolean }
    >
  >({});
  ...
  if (!quiz)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-foreground bg-background font-cairo">
        ...
        <p className="text-4xl font-black text-primary text-glow">
          {t("quiz.processing")}
        </p>
      </div>
    );
  ...
  const progress =
    ((currentQuestionIndex + (showResult ? 1 : 0)) / quiz.questions.length) *
    100;
  ...
  const isCoding =
    ["coding", "code", "programming"].includes(qType) ||
    (currentQuestion as any).isCoding ||
    ((currentQuestion as any).codeSnippet &&
      ["short-answer", "explanation"].includes(qType));
  const isExplain =
    [
      "explanation",
      "essay",
      ...
      (text) => {
        ...
        if (isMC || isTF) {
          ...
          for (const [idxStr, phrases] of Object.entries(matchers)) {
            ...
            if (
              phrases.some(
                (phrase) => lowerText.includes(phrase) || lowerText === phrase,
              )
            ) {
              matchedIdx = idx;
              break;
          ...
          if (!matchedOption && isTF) {
            if (
              lowerText.includes("صح") ||
              lowerText.includes("صحيح") ||
              lowerText.includes("true") ||
              lowerText.includes("يس")
            ) {
              matchedOption = language === "ar" ? "صح" : "True";
            } else if (
              lowerText.includes("خطأ") ||
              lowerText.includes("غلط") ||
              lowerText.includes("خاطئ") ||
              lowerText.includes("false") ||
              lowerText.includes("نو")
            ) {
      ...
      <div className="z-10 w-full max-w-5xl mx-auto mt-16">
        ...
        <Card className="bg-card/50 backdrop-blur-xl border-border/50 shadow-2xl rounded-[3rem] overflow-hidden">
          <CardHeader className="p-6 sm:p-10 md:p-16 pb-6 md:pb-8">
                ...
                return (
                  <button
                    ...
                    className={`
                                            ...
                                            ${
                                              isCurrent
                                                ? "ring-2 ring-primary ring-offset-2 scale-110 bg-primary text-primary-foreground shadow-lg"
                                                : isAnswered
                                                  ? quiz.questions[idx].type ===
                                                      "explanation" ||
                                                    [
                                                      "explanation",
                                                      "essay",
            ...
            <CardTitle
              className="text-2xl sm:text-3xl md:text-5xl font-black text-foreground leading-[1.3] text-center"
              dir="auto"
            >
            ...
            {(currentQuestion as any).codeSnippet && (
              <div
                className="mt-8 mx-auto w-full max-w-2xl bg-[#1e1e1e] rounded-2xl overflow-hidden shadow-2xl border-4 border-slate-800 text-left"
                dir="ltr"
              >
                ...
                <pre className="p-6 overflow-x-auto text-sm sm:text-base font-mono text-[#d4d4d4] leading-relaxed whitespace-pre-wrap">
                  <code>{(currentQuestion as any).codeSnippet}</code>
                </pre>
              </div>
            )}
          </CardHeader>
              ...
              {(isMC || isTF) &&
                (currentQuestion.options && currentQuestion.options.length > 0
                  ...
                  if (isAnswered) {
                    if (isCorrect)
                      btnClass =
                        "bg-green-500/20 border-green-500 text-green-600 shadow-sm";
                    else if (isSelected)
                      btnClass =
                        "bg-red-500/20 border-red-500 text-red-600 shadow-sm";
                    else
                      btnClass =
                        "bg-card border-border/30 text-foreground/30 opacity-60";
                  } else if (isSelected) {
                  ...
                  return (
                      ...
                      animate={
                        isAnswered && isCorrect
                          ...
                          : isAnswered && isSelected && !isCorrect
                            ? { x: [-5, 5, -5, 5, 0], scale: 0.98 }
                            : {}
                      }
                      whileHover={
                        !isAnswered
                          ? { scale: 1.02, x: language === "ar" ? 10 : -10 }
                          : {}
                      }
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center z-10 relative">
                        ...
                        <span className="flex-1 px-4 text-glow-sm">
                          {option}
                        </span>
              ...
              {isExplain && (
                <div className="space-y-4">
                  <textarea
                    ...
                    disabled={isAnswered}
                    rows={6} // Taller for code
                    defaultValue={currentAnswer?.textAnswer || ""}
                    className={cn(
                      "w-full p-6 leading-relaxed rounded-[2rem] text-lg font-mono bg-[#1e1e1e] border-4 transition-all outline-none resize-none text-[#d4d4d4] shadow-inner",
                      isAnswered
                        ? "border-primary/50 opacity-80"
                        : "border-slate-700 focus:border-primary focus:shadow-[0_0_20px_rgba(var(--primary),0.2)]",
                    )}
                    spellCheck={false}
                  />
                  {!isAnswered && (
              ...
              {isCoding && (
                <div className="space-y-4">
                  <div className="relative">
                    <div className="absolute top-0 right-0 p-2 text-xs text-muted-foreground bg-muted/20 rounded-bl-xl font-mono">
                      JS/TS
                    </div>
                    <textarea
                      ...
                      className={cn(
                        "w-full p-6 leading-relaxed rounded-xl font-mono text-sm sm:text-base bg-[#0f0f0f] text-[#d4d4d4] border-2 transition-all outline-none resize-none shadow-inner",
                        isAnswered
                          ? "border-primary/50 opacity-80"
                          : "border-[#333] focus:border-primary/50 focus:bg-[#151515]",
                      )}
                      spellCheck={false}
                    />
                  </div>
                  {!isAnswered && (
                    <Button
                      ...
                      className="w-full h-16 rounded-2xl text-xl font-black bg-slate-900 text-white hover:bg-black border border-slate-800"
                    >
                    ...
                    <div className="p-10 rounded-[2.5rem] bg-primary/5 border-2 border-primary/20 relative shadow-xl">
                      ...
                      <p className="text-foreground/80 text-xl leading-relaxed font-bold text-center mt-3">
                        {currentQuestion.explanation ||
                          t("quiz.defaultExplanation")}
                      </p>

                      {/* Coding Correct Answer Reveal */}
                      {isCoding && (
                        <div className="mt-8 text-left" dir="ltr">
                          ...
                          <div className="bg-[#1e1e1e] rounded-xl overflow-hidden border-2 border-green-500/20 shadow-lg relative group">
                            ...
                            <pre className="p-5 overflow-x-auto text-sm font-mono text-[#a6accd] leading-relaxed whitespace-pre-wrap selection:bg-green-500/30">

...
const AURA_TYPES = [
  {
    id: "night_owl",
    name: "Night Owl 🦉",
    desc: "You thrive when the world sleeps.",
  ...
  },
  {
    id: "early_bird",
    name: "Early Bird ☀️",
    desc: "Catching worms and high scores.",
  ...
  },
  {
    id: "brainiac",
    name: "The Brainiac 🧠",
    desc: "You eat textbooks for breakfast.",
  ...
  },
  {
    id: "survivor",
    name: "The Survivor 🛡️",
    desc: "You made it. That is what matters.",
...
export default function ResultsPage() {
  ...
  const [isSharingAura, setIsSharingAura] = useState(false);
  const [aura, setAura] = useState(AURA_TYPES[2]); // Default
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      const topicMap: Record<
        string,
        { total: number; correct: number; missedPages: Set<number> }
      > = {};
      results.questions.forEach((q: any, idx: number) => {
        const topic = q.topic || "عام";
        if (!topicMap[topic])
          topicMap[topic] = { total: 0, correct: 0, missedPages: new Set() };
      ...
      if (!localStorage.getItem(resultId)) {
        const xp = GamificationEngine.calculateQuizXP(
          results.score,
          results.total,
          120,
        ); // Dummy time for now
        ...
        setUnlockedBadges(
          updatedData.badges.filter(
            (b: any) => Date.now() - b.unlockedAt < 10000,
          ),
      ...
      // Calculate Aura
      const hour = new Date(results.timestamp).getHours();
      if (hour >= 0 && hour < 5)
        setAura(AURA_TYPES[0]); // Night Owl
      else if (hour >= 5 && hour < 9)
        setAura(AURA_TYPES[1]); // Early Bird
      else if (results.score / results.total >= 0.9)
        setAura(AURA_TYPES[2]); // Brainiac
      else if (results.score / results.total < 0.5) setAura(AURA_TYPES[4]); // Survivor
      // Random fallback for others for variety? Or just keep logic deterministic.
    }
  ...
  const shareOnTwitter = () => {
    ...
    window.open(
      `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent("https://mindar.tech")}`,
      "_blank",
    );
  };
  ...
  const copyToClipboard = () => {
    ...
    toast({
      ...
      description:
        language === "ar"
          ? "يمكنك الآن مشاركة الرابط مع أصدقائك!"
  ...
  const handleRevengeRound = () => {
    ...
    const wrongQuestions = data.questions.filter(
      (q: any, idx: number) => !data.answers[idx]?.isCorrect,
    );
    ...
    <main className="min-h-screen pt-40 px-6 bg-background font-cairo overflow-x-hidden pb-32">
      ...
      <div className="max-w-6xl mx-auto space-y-12 relative z-10 py-12">
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4 flex flex-wrap justify-center gap-4">
              <Link href="/hub">
                ...
                >
                  <RotateCcw
                    className={`w-5 h-5 transition-transform group-hover:rotate-180 ${language === "en" ? "rotate-180" : ""}`}
                  />
                  {t("hub.change")}
                </Button>
              </Link>
              ...
              {topics.some((t) => t.score < t.total) && (
                <Button
                  onClick={() =>
                    document
                      .getElementById("mistake-explorer")
                      ?.scrollIntoView({ behavior: "smooth" })
                  }
                ...
                <Button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      link.download = `mindar-aura-${Date.now()}.png`;
                      link.href = dataUrl;
                      link.click();
                    } catch (e) {
                      console.error(e);
                    } finally {
                      setIsSharingAura(false);
                    }
                  }}
                  className="w-full sm:w-auto h-14 px-8 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg font-black gap-2 animate-pulse"
                >
                  {isSharingAura ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            ...
            <h3 className="text-4xl font-black mb-2">
              {t("common.flashcards")} 🗂️
            </h3>
            <p className="text-xl opacity-90 font-bold">
              {t("common.flashcardsDesc")}
            </p>
            <div className="mt-8 bg-white/20 inline-block px-6 py-2 rounded-full font-black">
              {t("common.flashcardsBtn")}
            </div>
          </motion.div>
          ...
          <motion.div
            whileHover={{ scale: 1.02 }}
            onClick={() =>
              router.push(`/checklist/${data.materialId || data.id}`)
            }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <motion.div
            whileHover={{ y: -5 }}
            className="bg-card/50 backdrop-blur-xl border border-border/50 p-8 rounded-[2.5rem] text-center shadow-lg"
          >
            <p className="text-muted-foreground font-black text-sm uppercase mb-2">
              {t("common.score")}
            </p>
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h3 className="text-3xl font-black">
                  {t("results.foundMistakesTitle")}
                </h3>
                <p className="text-muted-foreground font-bold">
                  {t("results.foundMistakesDesc")}
                </p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {topics
                ...
                .map((topic, idx) => (
                  <div
                    key={idx}
                    className="bg-background/50 p-6 rounded-2xl border border-border/50 flex flex-col justify-between group transition-all hover:border-red-500/30"
                  >
                    <div>
                      ...
                      {topic.missedPages && topic.missedPages.length > 0 && (
                        <div className="flex flex-wrap gap-1 mb-4">
                          ...
                          {topic.missedPages.map((page: number) => (
                            <span
                              key={page}
                              className="px-2 py-0.5 bg-red-500/10 text-red-500 rounded-md text-[10px] font-black"
                            >
                    ...
                    <div className="flex items-center justify-between border-t border-border/50 pt-4 mt-2">
                      <span className="text-sm font-bold text-red-500">
                        {t("results.errorCount", {
                          count: topic.total - topic.score,
                        })}
                      </span>
                ...
                >
                  <div className="flex flex-col sm:flex-row items-start justify-between gap-4 mb-6">
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center font-black shrink-0 ${isCorrect ? "bg-green-500 text-white" : "bg-red-500 text-white"}`}
                      >
                        {idx + 1}
                      </div>
                      <h3
                        className="text-lg md:text-xl font-black leading-tight break-words"
                        dir="auto"
                      >
                        {q.question}
                      </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="p-4 rounded-2xl bg-background/50 border border-border/50">
                      ...
                      <p
                        className={`font-bold break-words ${isCorrect ? "text-green-600" : "text-red-600"}`}
                      >
                    ...
                    {!isCorrect && (
                      <div className="p-4 rounded-2xl bg-green-500/5 border border-green-500/20">
                        <p className="text-[10px] font-black uppercase text-green-600/60 mb-1">
                  ...
                  {q.explanation && (
                    <div className="p-6 rounded-2xl bg-primary/5 border border-primary/10">
                      <p className="text-xs font-black uppercase text-primary/60 mb-2">
                        {t("results.explanation")}
                      </p>
                      <p
                        className="text-sm font-medium opacity-80 italic break-words"
                        dir="auto"
                      >
                        {q.explanation}
                      </p>
                  ...
                  <AnimatePresence>
                    {explanations[idx] && (
                      ...
                      >
                        ...
                        <p
                          className="text-sm font-bold leading-relaxed text-indigo-900 dark:text-indigo-100"
                          dir="auto"
                        >
                          {explanations[idx]}
                        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-10">
          <Link href="/hub">
            <Button
              size="lg"
              className="w-full h-20 text-xl font-black rounded-2xl bg-primary text-primary-foreground shadow-xl"
            >
          ...
          <Button
            variant="outline"
            size="lg"
            className="w-full h-20 text-xl font-black rounded-2xl border-2 border-primary text-primary hover:bg-primary hover:text-white transition-all"
          >
            <Download className="mr-2 w-5 h-5" /> {t("common.export")}
          </Button>
          <Button
            size="lg"
            className="w-full h-20 text-xl font-black rounded-2xl bg-secondary text-white hover:bg-secondary/90 shadow-xl border-b-4 border-secondary/50 active:border-b-0 active:translate-y-1 transition-all"
          >
          ...
          <Link href="/">
            <Button
              variant="ghost"
              size="lg"
              className="w-full h-20 text-xl font-black rounded-2xl text-muted-foreground hover:bg-muted/50 hover:text-foreground"
            >
        ...
        {/* Hidden Aura Card Template */}
        <div style={{ position: "absolute", top: -9999, left: -9999 }}>
          <div
            ref={auraRef}
            className={`w-[1080px] h-[1920px] text-white p-12 flex flex-col relative overflow-hidden font-cairo bg-gradient-to-br ${aura.color}`}
          >
            {/* Noise Texture */}
            ...
            <div className="mt-20 text-center z-10">
              <p className="text-4xl font-bold uppercase tracking-[0.5em] opacity-80 mb-4">
                Mindar Wrapped
              </p>
              <h1 className="text-7xl font-black">My Study Aura</h1>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center z-10 relative">
              <div className="relative">
                ...
                <span className="text-[20rem] relative z-20 drop-shadow-2xl">
                  {aura.icon}
                </span>
              </div>

              <h2 className="text-[8rem] font-black mt-12 mb-4 text-center leading-none drop-shadow-2xl">
                {aura.name}
              </h2>
              <div className="h-2 w-40 bg-white rounded-full mb-8" />
              <p className="text-5xl font-bold text-center max-w-3xl leading-snug opacity-90">
                "{aura.desc}"
              </p>
            </div>

            {/* Stats Bubble */}
            <div className="z-10 bg-white/10 backdrop-blur-md rounded-[3rem] p-12 mb-20 border border-white/20 flex justify-between items-center">
              <div className="text-center">
                <p className="text-3xl font-bold opacity-60 uppercase mb-2">
                  Score
                </p>
              ...
              <div className="text-center">
                ...
                <p className="text-6xl font-black">+{earnedXP}</p>
              </div>
              <div className="w-[2px] h-20 bg-white/20" />
              <div className="text-center">
                <p className="text-3xl font-bold opacity-60 uppercase mb-2">
                ...
                <p className="text-4xl font-black max-w-[300px] truncate">
                  {data.title}
                </p>
              </div>
            </div>
            ...
            {/* Footer */}
            <div className="mb-20 flex items-center justify-center gap-4 z-10 opacity-70">

...
import { History, Calendar, Award, ChevronRight, Loader2, ArrowLeft, FileText, Trash2, Star } from 'lucide-react'
import Link from 'next/link'
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { cn } from "@/lib/utils"
import { getQuizHistory, clearQuizHistory, deleteQuizResult, QuizResult } from "@/lib/services/dbService"
  ...
  if (loading || authLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-slate-50 dark:bg-slate-950">
        ...
        <p className="font-black text-slate-500">جاري استرجاع ذكرياتك الدراسية...</p>
      </div>
    )
  }
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-4 text-center">
        ...
        <h1 className="text-3xl font-black text-slate-800 dark:text-slate-100">سجل الاختبارات متاح للأعضاء فقط</h1>
        <p className="text-slate-500 font-bold max-w-md">سجل دخولك الآن لتبدأ بحفظ تقدمك ومراجعة اختباراتك السابقة في أي وقت.</p>
        <Link href="/">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white">سجل اختباراتي</h1>
                    <p className="text-slate-500 font-bold">كل إنجازاتك محفوظة هنا يا بطل! 🔥</p>
                </div>
            </div>
            <div className="flex items-center gap-3">
                ...
                <Link href="/hub">
                    <Button variant="outline" className="h-12 px-6 rounded-xl font-black gap-2 border-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-12">
            <div className="p-6 bg-white dark:bg-slate-900 rounded-[2rem] border-2 border-slate-100 dark:border-slate-800 shadow-sm text-center">
                ...
                <p className="text-xs font-bold text-slate-400 mb-1">إجمالي الاختبارات</p>
                <p className="text-2xl font-black">{history.length}</p>
            </div>
            <div className="p-6 bg-white dark:bg-slate-900 rounded-[2rem] border-2 border-slate-100 dark:border-slate-800 shadow-sm text-center">
                <Award className="w-6 h-6 mx-auto mb-2 text-yellow-500" />
                <p className="text-xs font-bold text-slate-400 mb-1">متوسط الدرجات</p>
            ...
            <div className="p-6 bg-white dark:bg-slate-900 rounded-[2rem] border-2 border-slate-100 dark:border-slate-800 shadow-sm text-center">
                ...
                <p className="text-2xl font-black">
                    {history.length > 0 ? Math.max(...history.map(h => h.percentage)).toFixed(0) : 0}%
                </p>
            </div>
        </div>
        ...
        <div className="space-y-4">
                        ...
                        <Card className="rounded-[2rem] border-2 border-slate-100 dark:border-slate-800 hover:border-blue-500/30 transition-all group overflow-hidden">
                            <CardContent className="p-6 flex items-center gap-6">
                                <div className={cn(
                                    ...
                                    item.percentage >= 90 ? "bg-green-100 text-green-600 dark:bg-green-900/30" :
                                    item.percentage >= 60 ? "bg-blue-100 text-blue-600 dark:bg-blue-900/30" :
                                    "bg-red-100 text-red-600 dark:bg-red-900/30"
                                )}>
                                    <span className="text-xl">{item.score}</span>
                                    <span className="text-[10px] opacity-60">/{item.totalQuestions}</span>
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-4 text-xs font-bold text-slate-400">
                                        <div className="flex items-center gap-1">
                                            ...
                                            {item.timestamp?.toDate ? item.timestamp.toDate().toLocaleDateString('ar-EG') : 'تاريخ غير معروف'}
                                        </div>
                                        <div className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-[10px] uppercase">
                                            {item.quizData?.mode === 'challenge' ? 'تحدي' : 'اختبار فردي'}
                                        </div>
                                    </div>
                                </div>
            ...
            {history.length === 0 && (
                <div className="p-20 text-center bg-white dark:bg-slate-900 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                    ...
                    <p className="text-slate-400 font-bold">لا يوجد اختبارات سابقة حتى الآن. بادر بحل أول اختبار لتظهر نتيجتك هنا!</p>
                    <Link href="/hub">
                        <Button className="mt-6 rounded-xl font-black">ابدأ أول اختبار</Button>
                    </Link>
                </div>
            )}
        </div>

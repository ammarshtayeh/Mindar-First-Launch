...
import { ArrowLeft, ArrowRight, RotateCw, X, HelpCircle, CheckCircle2 } from 'lucide-react'
...
export function Flashcards({ cards, isOpen, onClose, isInline = false }: FlashcardsProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <div className={cn("w-full space-y-8", isInline ? "max-w-3xl" : "max-w-4xl")}>
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-3xl font-black">{t('common.flashcards')}</h2>
                  <p className="text-muted-foreground font-bold">
                    {language === 'ar' ? `البطاقة ${currentIndex + 1} من ${cards.length}` : `Card ${currentIndex + 1} of ${cards.length}`}
                  </p>
                </div>
              </div>
              <button 
            ...
            <div className="relative h-[450px] perspective-1000">
              ...
              >
                {/* Front Side */}
                <Card className={`absolute inset-0 w-full h-full backface-hidden p-12 flex flex-col items-center justify-center text-center rounded-[3.5rem] bg-card border-4 border-primary/20 shadow-2xl overflow-hidden`}>
                  ...
                  <p className="mt-8 text-primary font-black animate-bounce">
                    {language === 'ar' ? '(اضغط عشان تشوف الجواب)' : '(Click to see answer)'}
                  </p>
                </Card>

                {/* Back Side */}
                <Card 
                  className={`absolute inset-0 w-full h-full backface-hidden p-12 flex flex-col items-center justify-center text-center rounded-[3.5rem] bg-primary text-primary-foreground shadow-2xl overflow-hidden`}

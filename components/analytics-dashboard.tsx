...
export function AnalyticsDashboard({
  ...
  const [activePopup, setActivePopup] = React.useState<"radar" | "bar" | null>(
    null,
  );
  ...
  if (!isMounted) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full max-w-6xl mx-auto">
        <div className="h-[400px] bg-card/20 animate-pulse rounded-[3rem]" />
        <div className="h-[400px] bg-card/20 animate-pulse rounded-[3rem]" />
      </div>
    );
  }
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xl text-muted-foreground font-bold">
                    {t("medic.diagnosis")}:{" "}
                    {overallScore / totalQuestions >= 0.9
                      ? t("medic.good")
                      : overallScore / totalQuestions >= 0.7
                        ? t("medic.warn")
                        : t("medic.critical")}
                  </p>
                </div>
                ...
                <ul className="space-y-4">
                  {topicPerformance
                    ...
                    .map((topic, idx) => (
                      <li
                        key={idx}
                        className="flex items-start gap-3 bg-red-500/10 p-4 rounded-xl border border-red-500/20"
                      >
                  ...
                  {topicPerformance.filter((t) => t.percentage >= 70).length ===

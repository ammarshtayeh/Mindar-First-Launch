...
import { CheckCircle, XCircle, Timer, Award, ChevronRight, Loader2 } from "lucide-react"
...
export const ChallengeQuiz: React.FC<ChallengeQuizProps> = ({ roomId, userId, quizData, onFinish }) => {
        ...
        {!focusMode && (
          ...
          >
            <div className="flex items-center gap-2 bg-white dark:bg-slate-900 px-4 py-2 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
              ...
              <span className="font-black text-xl text-slate-900 dark:text-white">{score}</span>
            </div>
            
            <div className={cn(
              "flex items-center gap-2 px-6 py-2 rounded-2xl border transition-all",
              timeLeft < 5 ? "bg-red-50 border-red-200 text-red-500 animate-pulse" : "bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white"
            )}>
            ...
            <div className="bg-white dark:bg-slate-900 px-4 py-2 rounded-2xl border border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white">
               <span className="font-black opacity-40 text-xs ml-2 text-slate-500">سؤال</span>
               <span className="font-black">{currentIndex + 1} / {quizData.questions.length}</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            return (
              <Button
                ...
                className={cn(
                  ...
                  !isAnswered && "hover:border-red-500 hover:bg-red-50/50 dark:hover:bg-red-900/10 bg-slate-50 dark:bg-slate-800/40",
                  isAnswered && option === currentQuestion.answer && "bg-green-500/10 border-green-500 text-green-700 font-black opacity-100",
                  isAnswered && option !== currentQuestion.answer && "opacity-20"
                )}
              >
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="font-black text-lg text-black dark:text-slate-200">
                    {answers[currentIndex] ? "صح يا وحش! 🔥" : "للأسف غلط.. ركز! 🧐"}
                  </span>
                </div>
                <p className="text-slate-900 dark:text-slate-300 font-bold leading-relaxed break-words text-wrap">
                  {currentQuestion.explanation || "هذا السؤال يعتمد على فهمك للمادة، تأكد من مراجعته جيداً."}
                </p>

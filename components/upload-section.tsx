...
import { Upload, File, X, Loader2, XCircle, FileText, Zap, FileCheck } from "lucide-react"
                ...
                <CardContent className="p-12">
                     ...
                     >
                        <div className="w-24 h-24 bg-primary text-primary-foreground rounded-3xl flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-500">
                             {isProcessing ? <Loader2 className="w-12 h-12 animate-spin" /> : <Upload className="w-12 h-12" />}
                        </div>
                        <div className="text-center">
                            <h3 className="text-3xl font-black mb-2">{language === 'ar' ? 'ارفع مادتك' : t('upload.title')}</h3>
                            <p className="text-muted-foreground font-bold">{language === 'ar' ? 'اسحب الملفات هنا أو اضغط للاختيار' : t('upload.dragDrop')}</p>
                        </div>
                        <input 
                            type="file" 
                            ref={fileInputRef} 
                     ...
                     <AnimatePresence>
                        {files.length > 0 && (
                            ...
                            >
                                {files.map((file, idx) => (
                                    <div key={idx} className="flex items-center justify-between p-4 bg-background/50 rounded-2xl border border-primary/20">
                                        <div className="flex items-center gap-3">
                                            <FileText className="text-primary w-5 h-5" />
                                            <span className="font-bold text-sm truncate max-w-[200px]">{file.name}</span>
                                            <span className="text-[10px] opacity-40 font-black">{(file.size / 1024 / 1024).toFixed(2)} MB</span>
                                        </div>
                                        <button onClick={() => removeFile(idx)} className="p-2 hover:bg-red-500/10 text-red-500 rounded-xl transition-colors" disabled={isProcessing}>
                                            <X className="w-5 h-5" />
                                        </button>
                                    </div>
                                ))}
                                ...
                                >
                                    {isProcessing ? <Loader2 className="animate-spin" /> : <Zap className="fill-current w-6 h-6" />}
                                    {t('upload.prepare')}
                                </Button>
                            </motion.div>
                        )}
            ...
            {isProcessing && progress > 0 && (
                <div className="mt-4 px-10">
                    <div className="flex justify-between mb-2">
                        <span className="text-xs font-black opacity-60 uppercase">{language === 'ar' ? 'جاري التحضير...' : 'Preparing...'}</span>
